...
export default function HomePage() {
    ...
    useEffect(() => {
        const hasSwiped = localStorage.getItem("hasSwiped");
        if (!hasSwiped) {
    ...
    const handleSwipe = (_: any, info: any) => {
        // Velocity guard
        if (Math.abs(info.velocity.x) > 800) return;

        const threshold = 70;
        const currentIndex = toolsList.indexOf(tool);

        if (info.offset.x < -threshold && currentIndex < toolsList.length - 1) {
            setTool(toolsList[currentIndex + 1]);
            vibrate();
    ...
    return (
        <motion.div
            ...
            initial="hidden"
            animate="show"
            className="max-w-2xl mx-auto px-4 sm:px-6 py-6 sm:py-12 space-y-5 sm:space-y-8 relative pb-24 sm:pb-12"
        >
            <div className="space-y-4 text-center sm:mt-0">
                <div className="relative mb-2 sm:mb-6">
                    ...
                    {/* Desktop Toggle aligned with Switch */}
